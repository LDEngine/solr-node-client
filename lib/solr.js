...
Client.prototype.customsearch = function(query, handler, callback){
   var self = this;
   // Allow to be more flexible allow query to be a string and not only a Query object
   var parameters = query.build ? query.build() : query;
   var selectedquery = handler + '?';
   this.options.fullPath = [this.options.path,this.options.core,selectedquery + parameters + '&wt=json']
                              .filter(function(element){
                                 if(element) return true;
                                 return false;    
                              })
